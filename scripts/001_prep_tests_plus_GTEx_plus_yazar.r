...
##################
# Specific to this script - group with GTEx and Yazar too
##################
...
datasets = c("QTS000038", "QTS000015") # 015 is GTEx, 038 is Onek1k
external = do.call(rbind, lapply(datasets, function(x){
    dataset_name = eqtl_cat %>% 
        filter(study_id == !!x) %>%
        pull(study_label) %>%
        unique()

    print(paste0("..Getting eQTLs from - ", dataset_name))

    # Get ge datasets
    ge_datasets = eqtl_cat %>%
        filter(
            study_id == !!x,
            quant_method == "ge"
        ) %>% 
    ...
    ext_data = do.call(rbind, lapply(ge_datasets, function(y){
        # Get tissue
        tissue = eqtl_cat %>%
            filter(
                study_id == !!x,
        ...
        print(paste0("..Loading: ", tissue))

        # Load
        path = paste0(eqtldir, "/", x, "/", y, "/", y, ".permuted.tsv.gz")
        temp = read.delim(path) %>%
            mutate(
                qvalue = qvalue(p_beta)$qvalues, # Apply qvalue correction to beta-regressed pvalue
                type = paste0(!!dataset_name, "-", tissue)
            ) %>% 
            filter(
...
# Remove genes for which we do not get an eQTL (no point in comparing)
ourgenes = unique(coqtl$phenotype_id)
ours_external = ours_external %>% 
    ungroup() %>% 
    filter(
        phenotype_id %in% ourgenes
    )

# summary_genes
write.table(ours_external %>% pull(phenotype_id) %>% unique(), paste0(out.dir, "/gene_list.txt"), col.names=F, row.names=F, quote=F)
# variants per gene
write.table(ours_external, paste0(out.dir, "/variants_per_gene_list.txt"), col.names=T, row.names=F, quote=F, sep = "\t")
